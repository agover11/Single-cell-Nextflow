...
### Workflow Steps
1. **Download the data:**  
   - Use the GEO accession/EMBL-ENA to locate BAM files.  
...
Open a Jupyter notebook and select this environment as the kernel.
Introduction
Analyze two postnatal day 7 mouse hippocampus samples (from PMC Article).
Perform QC, clustering, and annotation of cell populations.
AnnData & Scanpy
AnnData stores a data matrix (adata.X), annotations (adata.obs/adata.var), and unstructured info (adata.uns).
Slice like a dataframe: adata_subset = adata[:, list_of_gene_names]
Basic Usage
import scanpy as sc
...
sc.pl.<tool_name>
Sample Files
Provided at: /projectnb/bf528/materials/single_cell/
Each sample is in a separate directory: filtered_feature_bc_matrix/
Citations
